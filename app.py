                ...
                try:
                    ...
                    if csv_url:
                        ...
                        if response.status_code == 200 and len(response.content) > 0:
                            # Read CSV
                            df_temp = pd.read_csv(io.StringIO(response.text), dtype=str).fillna('')
                            
                    # If CSV download returned empty or failed, try downloading entire XLSX as fallback
                    if df_temp is None or df_temp.empty or len(df_temp.columns) <= 1:
                        if sheet_id:
                            xlsx_url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/export?format=xlsx"
                            xlsx_resp = requests.get(xlsx_url, timeout=20)
                            if xlsx_resp.status_code == 200:
                                sheets_dict = pd.read_excel(io.BytesIO(xlsx_resp.content), sheet_name=None, dtype=str)
                                for name, df_sheet in sheets_dict.items():
                                    df_sheet = df_sheet.fillna('')
                                    # Use the first sheet that actually has columns and rows of data
                                    if len(df_sheet.columns) > 1 and len(df_sheet) > 0:
                                        df_temp = df_sheet
                                        used_fallback = True
                                        fallback_sheet_name = name
                                        break
                    ...
                    if df_temp is not None and not df_temp.empty:
                        ...
                        if used_fallback:
                            st.info(f"ℹ️ The specified Google Sheet tab was empty. Automatically fell back to the first non-empty tab: **'{fallback_sheet_name}'**.")
                        st.success("✅ Successfully fetched Google Sheet data!")
                    else:
                        st.error("Google Sheet returned no data (empty response). Please make sure the sheet is shared as 'Anyone with the link' (role: Viewer) and contains data.")
                except Exception as e:
